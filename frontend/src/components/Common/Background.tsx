...
export function IconParticleBackground({
  ...
  function spawnParticle(): Particle {
    ...
    return {
      ...
      size,
      alpha: 0,
      targetAlpha: parseFloat((Math.random() * 0.20 + 0.10).toFixed(2)),
  ...
  function edgeFade(vals: number[]): number {
  ...
  function animate() {
    ...
    for (let i = particlesRef.current.length - 1; i >= 0; i--) {
      ...
      // mouse magnetism
      p.translateX += (magnetRef.current.x / (ease / p.magnetism) - p.translateX) / ease;
      p.translateY += (magnetRef.current.y / (ease / p.magnetism) - p.translateY) / ease;

      // apply to DOM directly — no React re-render
      p.el.style.opacity = String(p.alpha);
      p.el.style.transform = `translate(${p.x + p.translateX}px, ${p.y + p.translateY}px) rotate(${p.rotation}deg)`;

      // respawn if off-screen
      if (
        p.x < -p.size || p.x > w + p.size ||
        p.y < -p.size || p.y > h + p.size
      ) {
        p.root.unmount();
        p.el.remove();
  ...
  useEffect(() => {
    // reinitialize when icon set changes (page change)
    init();
    const onResize = () => init();
  ...
  }, [icons, quantity]);  // icons identity changes → new particles
