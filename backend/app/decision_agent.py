...
async def make_decision(ctx: DecisionContext) -> DecisionResult:
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            _fill_governance(result, decision_id, ctx, attempt + 1,
                             int(time.monotonic() * 1000) - t0, None)
            _emit_audit(result)
            return result
            ...
            logger.warning(
                ...
                attempt + 1, MAX_RETRIES, ctx.document_id, exc,
            ...
            last_error = str(exc)
            logger.error("[DECISION_AGENT] non-retryable error doc=%s: %s",
                         ctx.document_id, exc)
            break

    # ── Deterministic fallback ─────────────────────────────────────────────────
    logger.warning("[DECISION_AGENT] deterministic fallback doc=%s reason=%s",
                   ctx.document_id, last_error)
    result = _deterministic_fallback(ctx)
    _fill_governance(result, decision_id, ctx, MAX_RETRIES,
                     int(time.monotonic() * 1000) - t0, last_error)
    _emit_audit(result)
    return result
...
def _parse_response(
    ...
    return DecisionResult(
        ...
        recommended_action=str(parsed.get("recommended_action", "Requires human review"))[:300],
        # Governance filled by caller
        decision_id="", timestamp="", document_id="", org_id="",
        triggered_by="", role="",
        method="ai_agent",
        model_used=settings.CHUTES_MODEL,
        attempt_count=0, latency_ms=0, fallback_reason=None,
    )
...
def _emit_audit(result: DecisionResult) -> None:
    ...
    logger.info(
        ...
        result.document_id, result.decision, result.auto_approve,
        result.confidence, result.risk_score, result.risk_level,
        result.method, result.attempt_count, result.latency_ms,
    )
