...
async def list_decision_audit_log(
    ...
    document_id: str | None = Query(default=None, description="Filter by document UUID"),
    org_id: str | None = Query(default=None, description="Filter by organization ID"),
    decision: str | None = Query(default=None, description="Filter by decision: approved|exception|escalated|rejected"),
    method: str | None = Query(default=None, description="Filter by method: ai_agent|deterministic_fallback"),
    limit: int = Query(default=50, ge=1, le=500, description="Max entries to return (newest first)"),
...
async def get_document_decisions(
    ...
    return AuditLogResponse(entries=entries[:limit], total=len(entries), filtered=min(len(entries), limit))
...
async def decision_stats_summary(current_user: CurrentUser):
    ...
    if not entries:
        return {"total": 0, "by_decision": {}, "by_method": {}, "by_risk_level": {},
                "auto_approve_rate": 0.0, "fallback_rate": 0.0, "avg_confidence": 0.0,
                "avg_risk_score": 0.0, "avg_latency_ms": 0.0}
