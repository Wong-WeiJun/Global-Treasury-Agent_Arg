...
async def orchestrated_reconcile(
    ...
    best_idx = (
        final_decision.get("matched_entry_index")
        or orch_agent_decision.get("matched_entry_index")
    )
    best_score_dict = (
    ...
    if decision_result.auto_approve:
        ...
        if status == "matched_with_adjustment" and journal_entry_proposal:
            ...
            journal_entry = generate_journal_entry(proof_ctx, fx_result_ctx, "approved", None)
            exception_type = None
            action = "approved"
            note = (
